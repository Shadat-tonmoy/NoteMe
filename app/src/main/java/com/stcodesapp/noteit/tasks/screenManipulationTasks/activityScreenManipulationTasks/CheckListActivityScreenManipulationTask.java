...
public class CheckListActivityScreenManipulationTask {

    private Activity activity;
    private CheckListScreenView checkListScreenView;
    private ClipboardTasks clipboardTasks;

    public CheckListActivityScreenManipulationTask(Activity activity, ClipboardTasks clipboardTasks) {
    ...
    public void bindCheckListObject(List<ChecklistItem> checklistItems) {
        if (checklistItems.size() > Constants.ZERO) {
            checkListScreenView.getCheckListItemAdapter().bindCheckListObjects(checklistItems);
        }
    }
    ...
    public void addEmptyChecklistItem() {
            checkListScreenView.getCheckListItemAdapter().addEmptyChecklistItem();
        final int lastIndex = checkListScreenView.getCheckListItemAdapter().getItemCount()-1;
...
//                clipboardTasks.showKeyBoard();
    ...
    public CheckList grabCheckListValue()
    {
        ...
        checkList.setChecklistItems(checkListScreenView.getCheckListItemAdapter().getCheckListObjects());
        int totalItems = checkListScreenView.getCheckListItemAdapter().getItemCount();
        try {
            ...
            checkListScreenView.getCheckListItemAdapter().getCheckListObjects().get(totalItems-1).setField1(lastFieldValue);
        }catch (Exception e)
        {
            e.printStackTrace();
        }
        try {
            for(int i=0;i<totalItems;i++)
            {
                ...
                checkListScreenView.getCheckListItemAdapter().getCheckListObjects().get(i).setField1(fieldValue);
            }
